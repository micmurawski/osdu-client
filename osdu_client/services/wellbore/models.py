...
#   timestamp: 2024-07-12T07:32:23+00:00
...
from pydantic import BaseModel, Field, confloat, RootModel, ConfigDict
...
class Trajectorychannel(BaseModel):
    ...
    bulkURI: Optional[str] = Field(
        None,
        json_schema_extra=dict(description="bulkURI either URL or URN.", title="bulk URI")
    )
    ...
    dimension: Optional[int] = Field(
        None,
        json_schema_extra=dict(
            description="The dimension of this log or channel", title="Dimension"
        )
    )
    ...
    longName: Optional[str] = Field(
        None,
        json_schema_extra=dict(
            description="The long name of this log or channel", title="Log Long Name"
        )
    )
    mnemonic: Optional[str] = Field(
        None,
        json_schema_extra=dict(
            description="The mnemonic of this log or channel", title="Mnemonic"
        )
    )
    name: Optional[str] = Field(
        None,
        json_schema_extra=dict(
            description="The name of this log or channel.", title="Log Name"
        )
    )
    properties: Optional[List[str]] = Field(
        ...
        json_schema_extra=dict(
            description="The properties of this log or channel.", title="Properties"
        )
    )
...
class Wellborerelationships(BaseModel):
    ...
    well: Optional[ToOneRelationship] = Field(
        None,
        json_schema_extra=dict(
            description="The well to which this wellbore belongs.", title="Well"
        )
    )
...
class Wellrelationships(BaseModel):
    asset: Optional[ToOneRelationship] = Field(
        None,
        json_schema_extra=dict(
            description="The asset this well belongs to.", title="Asset"
        )
    )
...
class BulkDataStatisticsResponse(BaseModel):
    ...
    recordId: str = Field(..., json_schema_extra=dict(title="Recordid"))
    recordVersion: int = Field(..., json_schema_extra=dict(title="Recordversion"))


class Catalog(BaseModel):
    family_catalog: List[CatalogItem] = Field(..., json_schema_extra=dict(title="Family Catalog"))
    main_family_catalog: Optional[List[MainFamilyCatalogItem]] = Field(
        None, json_schema_extra=dict(title="Main Family Catalog")
    )
...
class CommitSessionResponse(BaseModel):
    createdTime: datetime = Field(..., json_schema_extra=dict(description="creation date", title="Createdtime"))
    ...
    state: SessionState
    updatedTime: datetime = Field(..., json_schema_extra=dict(description="updated date", title="Updatedtime"))
...
class CreateDataSessionRequest(BaseModel):
    ...
    meta: Optional[Dict[str, str]] = Field(
        None, json_schema_extra=dict(description="dictionary all values, stored in the session", title="Meta")
    )
    mode: SessionUpdateMode = Field(
        ...,
        json_schema_extra=dict(
    ...
    timeToLive: Optional[int] = Field(
        1440, json_schema_extra=dict(description="optional - time to live in minutes.", title="Timetolive")
    )
...
class Dip(BaseModel):
    ...
    classification: Optional[str] = Field(
        None,
        json_schema_extra=dict(
            description="Any string is accepted.", title="Classification of the dip"
        )
    )
    ...
    quality: Optional[ValueWithUnit] = Field(
        None,
        json_schema_extra=dict(
            description="Decimal number between 0 and 1", title="Quality of the dip"
        )
    )
